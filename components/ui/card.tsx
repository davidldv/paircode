...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn("rounded-xl border border-black/10 bg-white shadow-sm dark:border-white/20 dark:bg-black/30", className)}
      {...props}
    />
  );
}
...
function CardDescription({ className, ...props }: React.ComponentProps<"p">) {
  return <p data-slot="card-description" className={cn("text-sm text-black/70 dark:text-white/70", className)} {...props} />;
}
