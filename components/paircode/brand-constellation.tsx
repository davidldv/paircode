...
export function BrandConstellation({ compact = false, className }: BrandConstellationProps) {
  return (
    <section
      className={cn(
        "border border-(--panel-border) bg-(--surface) p-6 shadow-sm rounded-xl transition-all duration-300 hover:shadow-md animate-fade-in",
        className,
      )}
    >
      <div className={cn("flex gap-4", compact ? "items-center" : "items-start")}>
        <div
          className={cn(
            "shrink-0 bg-(--surface-strong) rounded-xl flex items-center justify-center transition-all duration-500 hover:scale-105 hover:rotate-3",
            compact ? "p-2.5" : "p-4",
          )}
        >
          <img
      ...
      <div className={cn("mt-8 grid gap-4", compact ? "md:grid-cols-2 xl:grid-cols-4" : "sm:grid-cols-2")}>
        {capabilityCards.map((card, i) => (
          <article
            key={card.title}
            className="group relative overflow-hidden border border-(--panel-border) bg-(--surface) p-4 rounded-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-(--accent-glow) animate-slide-up"
            style={{ animationDelay: `${i * 100}ms`, animationFillMode: "both" }}
          >
            <div className="absolute inset-0 bg-gradient-to-br from-(--accent-glow) to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
            <div className="relative flex flex-col gap-3">
              <div className="w-10 h-10 rounded-lg bg-(--surface-strong) flex items-center justify-center text-(--accent) transition-transform duration-300 group-hover:scale-110">
